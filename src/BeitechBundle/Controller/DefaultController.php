...
//            ejecutar una acción que le diga al usuario que no se pudo subir el vídeo
        ...
        if ($request->isXMLHttpRequest()) {
            ...
            $response[] = array(
                ...
                'user_id_login22' => $user_id_login22
            );
            return new Response(json_encode($response), 200, array('Content-Type' => 'application/json'));
        }
    ...
    public function uploadKeywordsFormAction(Request $request) {
        ...
        $keywordFour = $_POST['keywordFour'];
        $keywordFive = $_POST['keywordFive'];
        $productId = $_POST['productId'];
        
        $em = $this->getDoctrine()->getManager();

        if ($request->isXMLHttpRequest()) {
            $keywordOneObject = new Keyword();
            $keywordOneObject->setName($keywordOne);
            $em->persist($keywordOneObject);
            $em->flush();
            $keywordOneObjectId = $keywordOneObject->getId();
            $keywordOneObjectEntity = $em->getRepository('BeitechBundle:Keyword')->findOneById($keywordOneObjectId);
            
            
            $keywordTwoObject = new Keyword();
            $keywordTwoObject->setName($keywordTwo);
            $em->persist($keywordTwoObject);
            $em->flush();
            $keywordTwoObjectId = $keywordTwoObject->getId();
            $keywordTwoObjectEntity = $em->getRepository('BeitechBundle:Keyword')->findOneById($keywordTwoObjectId);
            
            $keywordThreeObject = new Keyword();
            $keywordThreeObject->setName($keywordThree);
            $em->persist($keywordThreeObject);
            $em->flush();
            $keywordThreeObjectId = $keywordThreeObject->getId();
            $keywordThreeObjectEntity = $em->getRepository('BeitechBundle:Keyword')->findOneById($keywordThreeObjectId);
            
            $keywordFourObject = new Keyword();
            $keywordFourObject->setName($keywordFour);
            $em->persist($keywordFourObject);
            $em->flush();
            $keywordFourObjectId = $keywordFourObject->getId();
            $keywordFourObjectEntity = $em->getRepository('BeitechBundle:Keyword')->findOneById($keywordFourObjectId);
            
            $keywordFiveObject = new Keyword();
            $keywordFiveObject->setName($keywordFive);
            $em->persist($keywordFiveObject);
            $em->flush();
            $keywordFiveObjectId = $keywordFiveObject->getId();
            $keywordFiveObjectEntity = $em->getRepository('BeitechBundle:Keyword')->findOneById($keywordFiveObjectId);
            
            $productIdEntity = $em->getRepository('BeitechBundle:Product')->findOneById($productId);
            
            $productKeywordOneObject = new ProductKeyword();
            $productKeywordOneObject->setIdKeyword($keywordOneObjectEntity);
            $productKeywordOneObject->setIdProduct($productIdEntity);
            $em->persist($productKeywordOneObject);
            $em->flush();
            
            $productKeywordTwoObject = new ProductKeyword();
            $productKeywordTwoObject->setIdKeyword($keywordTwoObjectEntity);
            $productKeywordTwoObject->setIdProduct($productIdEntity);
            $em->persist($productKeywordTwoObject);
            $em->flush();
            
            $productKeywordThreeObject = new ProductKeyword();
            $productKeywordThreeObject->setIdKeyword($keywordThreeObjectEntity);
            $productKeywordThreeObject->setIdProduct($productIdEntity);
            $em->persist($productKeywordThreeObject);
            $em->flush();
            
            $productKeywordFourObject = new ProductKeyword();
            $productKeywordFourObject->setIdKeyword($keywordFourObjectEntity);
            $productKeywordFourObject->setIdProduct($productIdEntity);
            $em->persist($productKeywordFourObject);
            $em->flush();
            
            $productKeywordFiveObject = new ProductKeyword();
            $productKeywordFiveObject->setIdKeyword($keywordFiveObjectEntity);
            $productKeywordFiveObject->setIdProduct($productIdEntity);
            $em->persist($productKeywordFiveObject);
            $em->flush();
...
//            $em->flush();
            ...
            $response = array();
            $response[] = array(
                'variable1' => $keywordOneObjectId,
                'variable2' => $keywordTwoObjectId,
                'variable3' => $keywordThreeObjectId,
                'variable4' => $keywordFourObjectId,
                'variable5' => $keywordFiveObjectId
//                'product_id' => $product->getId(),
//                'product_name' => $product->getName(),
//                'product_description' => $product->getDescription(),
//                'product_price' => $product->getPrice(),
//                'product_image1' => $product->getImagePart1(),
//                'product_image2' => $product->getImagePart2()
            );
            return new Response(json_encode($response), 200, array('Content-Type' => 'application/json'));
        }
    }
